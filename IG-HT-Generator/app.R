...
hashtags <- data.frame(hashtag = c("#california",
  "#cali",
  "#goldenstate",
  "#caligrammers",
  "#californiaholics",
...
ui <- fluidPage(
   ...
   fluidRow(
     column(12,
            checkboxGroupInput("theme_select", "theme", choices = themes, selected = "general")
     )
   ),
   fluidRow(
     column(12,
    ...
    observe({
      ...
      choose_tags <- function(chosen_themes, n){
        ...
        chosen_themes <- chosen_themes[sample(1:n, size = n, replace = FALSE)]
          ...
          for (i in 1:n) {
            filtered_df <- subset(hashtags, subset = hashtags$theme %in% chosen_themes[i])
        ...
        if (length(input$theme_select) == 0) {
          ...
          n = 1
          chosen_hashtags <- choose_tags(chosen_themes = chosen_themes, n = n)
          } else {
          chosen_hashtags <- choose_tags(chosen_themes = unique(input$theme_select), n = n)
          }
        
        
        output$chosen_hashtags <- renderText({
          chosen_hashtags <- paste(chosen_hashtags, collapse = " ")
          final_text <- paste0(input$post_text,"\n.\n.\n.\n.\n",chosen_hashtags)
          paste(final_text)
        })
